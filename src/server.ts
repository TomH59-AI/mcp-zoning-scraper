...
function createServer(): McpServer {
  const server = new McpServer({ name: "mcp-zoning-scraper", version: "2.1.0" });
...
function startHttpServer(port: number, authToken: string): void {
  ...
  app.get("/health", (_request: Request, response: Response) => {
    response.status(200).json({ ok: true, service: "mcp-zoning-scraper", version: "2.1.0", active_job: activeJob ? jobView(activeJob) : null });
  });
